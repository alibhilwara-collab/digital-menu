...
export default function EditMenuPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [itemForm, setItemForm] = useState<EditItem>({
    id: "",
    name: "",
    price: "",
    description: "",
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: menu } = await supabase
        ...
        .eq("id", id)
        .single()
      if (!menu) { router.push("/dashboard/menus"); return }
      setMenuName(menu.name)
      setMenuDescription(menu.description || "")
      setCoverImage(menu.cover_image || "")
      setWhatsappNumber(menu.whatsapp_number || "")
      ...
      setCategories(
        (cats || []).map((c: { id: string; name: string; items: { id: string; name: string; price: number; description: string | null; image: string | null; available: boolean }[] }) => ({
          id: c.id,
          name: c.name,
          expanded: true,
          items: (c.items || []).map((i: { id: string; name: string; price: number; description: string | null; image: string | null; available: boolean }) => ({
            id: i.id,
            name: i.name,
  ...
  function addCategory() {
    ...
    setCategories([
      ...categories,
      { id: `cat-${Date.now()}`, name: newCategoryName.trim(), items: [], expanded: true },
    ])
    setNewCategoryName("")
  }
  ...
  function toggleCategory(catId: string) {
    setCategories(categories.map((c) => (c.id === catId ? { ...c, expanded: !c.expanded } : c)))
  }

  function openAddItem(catId: string) {
    setAddItemCatId(catId)
    setItemForm({ id: `item-${Date.now()}`, name: "", price: "", description: "", image: "", available: true })
  }
  ...
  function saveItem() {
    ...
    setCategories(
      categories.map((c) =>
        c.id === addItemCatId ? { ...c, items: [...c.items, { ...itemForm }] } : c
      )
    )
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { error: menuErr } = await supabase
        .from("menus")
        .update({ name: menuName.trim(), description: menuDescription.trim() || null, cover_image: coverImage || null, whatsapp_number: whatsappNumber.trim() || null })
        .eq("id", id)
      if (menuErr) throw menuErr

      // Delete old categories & items, then re-create
      await supabase.from("categories").delete().eq("menu_id", id)

      for (let i = 0; i < categories.length; i++) {
        const cat = categories[i]
        const { data: catData, error: catErr } = await supabase
          .from("categories")
          .insert({ menu_id: id, name: cat.name, sort_order: i })
          .select()
          .single()
  ...
  if (loading) {
    return (
      <div className="flex flex-col">
        <DashboardHeader title="Edit Menu" />
      ...
      <div className="flex-1 p-4 lg:p-8">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold tracking-tight text-foreground">Edit: {menuName || "Menu"}</h2>
              <p className="text-sm text-muted-foreground">Update your menu details and items</p>
            </div>
          </div>
          <Link href={`/menu/${id}`}>
                      ...
                      <Dialog open={addItemCatId === cat.id} onOpenChange={(open) => !open && setAddItemCatId(null)}>
                        ...
                        <DialogContent className="sm:max-w-md">
                          ...
                          <div className="space-y-4 py-2">
                            ...
                            <Button onClick={saveItem} className="w-full">Add Item</Button>
                          </div>
                        </DialogContent>
                      </Dialog>
                    </div>
